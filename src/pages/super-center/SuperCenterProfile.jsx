...
export default function SuperCenterProfile() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (!user) return
    supabase.from('centers').select(SELECT)
  ...
  if (!center) return (
    <div className="p-6"><p className="text-gray-500">No super center profile found.</p></div>
  )
