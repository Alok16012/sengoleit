...
export default function SuperCenterStudents() {
  ...
  async function fetchStudents(cid) {
    ...
    const { data } = await supabase
      .from('students')
      .select('id, student_name, enrollment_no, mobile_no, gender, status, entry_type, programs(program_name), academic_sessions(session_name)')
      .eq('center_id', cid)
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <Table>
          ...
          <Tbody>
            {filtered.length === 0 ? (
              <Tr><Td colSpan={9} className="text-center text-gray-400 py-12">No students found</Td></Tr>
            ) : filtered.map((s, i) => (
              <Tr key={s.id}>
