...
export default function StudentAdmissionForm() {
  ...
  const addr = [
    data.student_perm_village_town,
    data.student_perm_city,
    data.student_perm_district,
    data.student_perm_state,
  ].filter(Boolean).join(', ') || '—'
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-gray-900 flex items-center gap-2"><ClipboardList size={20} className="text-[#933d18]" /> Admission Form</h1>
          <p className="text-xs text-gray-400 mt-0.5">Download your admission form</p>
        </div>
        <button
        ...
        >
          <Download size={15} /> {generating ? 'Generating...' : 'Download Admission Form'}
        </button>
      </div>
      ...
      <div className="bg-white border-2 border-gray-300 rounded-2xl overflow-hidden shadow-sm">
        <div className="bg-[#933d18] text-white text-center py-3">
          <span className="font-black text-base tracking-widest">ADMISSION FORM</span>
        </div>
        ...
        <div className="flex gap-0">
          <div className="flex-1 p-5 space-y-2.5">
            {[
              ...
              { icon: BookOpen, label: 'Session', value: data.academic_sessions?.session_name || data.academic_year },
              { icon: User, label: 'Student Name', value: data.student_name },
              { icon: Hash, label: 'Date of Birth', value: formatDate(data.date_of_birth) },
              { icon: User, label: "Father's Name", value: data.fathers_name },
              { icon: MapPin, label: 'Address', value: addr },
              { icon: Hash, label: 'Mobile No', value: data.mobile_no },
            ].map(({ label, value }) => (
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-xs text-emerald-700">
        This is your admission form for Sengol International University. Download and keep a printed copy for your records.
      </div>
    </div>
  )
}
