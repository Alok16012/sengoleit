...
import { Plus, Search, X, Check, Trash2, Building2, GraduationCap, CheckCircle2, Clock } from 'lucide-react'
...
export default function CenterCourses() {
  ...
  // filters (shared by the Add Course catalog)
  const [search, setSearch]   = useState('')
  const [fDept, setFDept]     = useState('all')
  const [fType, setFType]     = useState('all')
  const [fSession, setFSession] = useState('all')

  const [busy, setBusy] = useState(null) // fee_structure_id currently saving
  ...
  // Load allotments whenever a center is picked.
  useEffect(() => {
  ...
  // Catalog rows after filters (used in Add Course panel).
  const catalog = structs.filter(s => {
  ...
  // Allotted rows for the active status sub-tab.
  const allottedRows = structs
    .filter(s => allot[s.id] && allot[s.id].status === subTab)
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-5 shadow-sm">
        <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1">Select Center</label>
        <select
          value={centerId}
          onChange={e => { setCenterId(e.target.value); setAdding(false); setSubTab('pending') }}
          className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-semibold text-gray-700 bg-white min-w-[280px] focus:outline-none focus:ring-2 focus:ring-[#933d18]/20"
        >
        ...
        {/* Center detail (auto) */}
        {center && (
          <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-1 border-t border-gray-100 pt-4 text-sm">
            <div className="flex items-center gap-2 text-gray-800 font-bold">
              <Building2 size={15} className="text-[#933d18]" /> {center.center_name}
            </div>
      ...
      {!centerId ? (
        <div className="flex flex-col items-center justify-center py-24 text-gray-300">
          ...
          <p className="text-base font-semibold text-gray-400">Select a center to allot courses</p>
        </div>
      ) : (
        <>
          ...
          {/* ── ADD COURSE PANEL: filters + catalog with tick/untick ── */}
          {adding ? (
            <>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {catalog.length === 0 ? (
                      <tr><td colSpan={6} className="text-center text-gray-400 py-12">No courses match the filters</td></tr>
                    ) : catalog.map((s, i) => {
                      const a = allot[s.id]
                      const checked = !!a
                      return (
