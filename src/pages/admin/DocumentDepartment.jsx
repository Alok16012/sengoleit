...
export default function DocumentDepartment() {
  ...
  useEffect(() => { if (mainTab === 'centers') fetchCenterApps() }, [mainTab])
      ...
      <div className="flex gap-1 mb-4 bg-gray-100 p-1 rounded-xl w-fit">
        {MAIN_TABS.map(t => (
          <button key={t.key} onClick={() => setMainTab(t.key)}
            ...
            {t.label}
            {t.key === 'centers' && centerApps.length > 0 && (
              <span className="ml-2 bg-amber-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{centerApps.length}</span>
            )}
          </button>
        ))}
      </div>
