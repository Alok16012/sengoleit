...
-- v_miscounted_coupons, and a Postgres view runs with its OWNER's rights unless
-- told otherwise — so it handed the whole coupons table to anon and undid the
-- RLS that security_hardening.sql put there. The line below removes it, and the
-- batch is now selected inline instead.
-- ============================================================

DROP VIEW IF EXISTS v_miscounted_coupons;

-- ------------------------------------------------------------
-- Edit these three values in BOTH steps to describe the batch:
--   center_code   the centre
--   face_value    the wrong denomination that was minted
--   created_at    the date the batch was made
-- ------------------------------------------------------------
...
WITH batch AS (
  SELECT
    c.id, c.face_value, ctr.center_name, ctr.center_code, ctr.coupon_wallet_balance,
...
-- ============================================================
-- STEP 2 — apply. Uncomment this whole block and run it.
...
-- Confirm before committing: the wallet should be back up, and the count 0.
...
WHERE ctr.center_code = 'SIU002' AND c.face_value = 25 AND c.created_at::date = DATE '2026-08-04';
